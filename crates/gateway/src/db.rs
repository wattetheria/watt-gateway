...
pub async fn init_schema(pool: &PgPool) -> Result<()> {
    ...
    backfill_mission_lifecycle_projection_rows(pool).await?;

    Ok(())
}

async fn backfill_mission_lifecycle_projection_rows(pool: &PgPool) -> Result<()> {
    sqlx::query(
        r#"
        ...
        select
            ...
            payload
                || jsonb_build_object(
                    ...
                    'publisher_wattswarm_node_id', node_id,
                    'mission_scope_hint', 'node:' || node_id,
                    'mission_feed_key', 'wattetheria.missions',
                    'status',
                        case
                            when event_kind = 'mission.claimed' then 'claimed'
                            when event_kind = 'mission.completed' then 'completed'
                            when event_kind = 'mission.settled' then 'settled'
                            when coalesce(nullif(payload->>'status', ''), 'open') = 'open'
                                then 'published'
                            else payload->>'status'
                        end
                ),
            jsonb_build_object(
                'source_node_id', node_id,
                'source_cursor_or_seq', source_cursor_or_seq,
                'ingest_path', 'event_push_backfill',
                'last_confirmed_at', case when provisional then null else extract(epoch from generated_at)::bigint end,
                'last_provisional_at', case when provisional then extract(epoch from generated_at)::bigint else null end
            )
        from (
            select
                *,
                coalesce(
                    nullif(payload->>'mission_id', ''),
            ...
            from gateway_ui_events
            where data_kind = 'mission_lifecycle'
        ) mission_events
